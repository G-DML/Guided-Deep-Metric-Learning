...
class TripletNet_conv(nn.Module):
    def __init__(self, in_size=2, out_size=2, num_classes=2, m=1.0, beta=0.0, pow=2):
        super().__init__()
        self.beta = beta
        self.convnet = nn.ModuleList([nn.Sequential(nn.Conv2d(1, 32, 5), nn.PReLU(), #nn.Conv2d(in_channels, out_channels, kernel_size)
                                     nn.MaxPool2d(2, stride=2),
                                     nn.Conv2d(32, 64, 5), nn.PReLU(),
                                     nn.MaxPool2d(2, stride=2)) for _ in range(num_classes)])
        self.fc = nn.Sequential(nn.Linear(64 * 4 * 4, 256), #nn.Linear(1024, 256)
                                nn.PReLU(),
                                nn.Linear(256, 256),
    ...
    def get_new_Dataset(self, dataset, device):
        ...
        with torch.no_grad():
            for i, c in enumerate(labels_set):
                indexes = dataset.targets.numpy() == c

                data_subset = dataset.data[indexes]
                l, w, d = data_subset.shape
                data_subset = data_subset.view(l, 1, d, w)

                labels_subset = dataset.targets[indexes]
...
def ResNet_size(size=10, in_channels=1, output_size=2):
    if size == 10:
...
def fit(epochs, max_lr, model, train_loader, val_loader, weight_decay=0.0, grad_clip=None, opt_func=torch.optim.SGD):
    ...
    sched = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.60, patience=1, verbose=True)
...
# Model (on Device)
resnet_model = to_device(ResNet_size(size=18, in_channels=1, output_size=output_dim), device)

# Batch size
batch_size = 32

#Dataset to Batches
resnet_train_ld = DataLoader(train_ds, batch_size=batch_size, shuffle=True)  # shuffle=True
...
#File_name = "Log/GEMINI/Fashion/" + str(output_dim) + "d/Fashion_rec-" + str(recall) + "_bs-" + str(batch_size) + "_eps-" + str(epochs) + "_lr-" + str(max_lr) + "_m-" + str(margin) + ".pth"
