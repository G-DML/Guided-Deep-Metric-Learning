...
def fit(epochs, max_lr, model, train_loader, val_loader, weight_decay=0.0, grad_clip=None, opt_func=torch.optim.SGD):
    ...
    sched = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.80, patience=1, verbose=True)
...
#File_name = "Log/GEMINI/MNIST/" + str(output_dim) + "d/MNIST_rec-" + str(recall) + "_bs-" + str(batch_size) + "_eps-" + str(epochs) + "_lr-" + str(max_lr) + "_m-" + str(margin) + ".pth"
